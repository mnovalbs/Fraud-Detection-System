...
class Fds extends CI_Controller {
  ...
  $date_now = $this->input->post('order_date');
  
  $this->load->model('fds_model');
  $ip_prev = $this->fds_model->get_prev_ip();
  $date_prev =  $this->fds_model->get_prev_date();
  
  $interval = abs(strtotime($date_now) - strtotime($date_prev));
  $interval = $interval/60/60
  
  if ( ($ip_prev != $ip_now) && ($interval>12) ){
  	return true;
    }
  ...
  $this->fds_model->get_last_three_days()
